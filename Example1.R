...
eps_vec <- c(0.2)
#eps_vec <- c(0.1, 0.2, 0.3, 0.25, 0.15, 0.35)
#eps_vec <- seq(from = 1, to = 0.005, by = -0.005) 
...
#----------
tic("calculation of preliminary estimator")
prelim_est_list <- c()
...
tic("calculation of solution of Riccati equation")
...
for(epsilon in eps_vec){
  ...
  for(upper_bound in (ceiling((tau_eps_rounded*1000/3)/3)*3):1000){
    value_integral <- 0
    for(i in 1:(upper_bound-round((epsilon^(1/6))/3*1000))){
...
for(epsilon in eps_vec){
  ...
  if(round(tau_eps,3)*1000/3+2<1000){
    number_of_simulation_steps <- 1000-ceiling(round(epsilon^(1/6),3)*1000/3)
...
plot_consistency_wrt_t_full <- ggplot(df_consistency_wrt_t_list[[as.character(0.315)]], aes(x=times)) +
...
plot_consistency_wrt_t_partially <- ggplot(df_consistency_wrt_t_list[[as.character(0.315)]], aes(x=times)) +
  geom_line(aes(y=values_prelim,linetype="solid"))+ #It should be the other way around, but this works
  geom_line(aes(y=values_one_step,linetype="dotted"))+
  geom_line(aes(y=values_real,linetype="dashed"))+
  ylim(0.8,1.5)+
...
for(epsilon in eps_vec){
  print(histogram_list[[as.character(epsilon)]])
}
#-> choose epsilon = 0.32!
#set up QQ-Plot to (visually) check for normality
ggplot(trimmed_values_for_histogram[[as.character(0.32)]], aes(sample=values)) +
...
#----------
# Create plot for X_t and Y_t visualization
#----------
process_x_data_frame <- data.frame(x_for_epsilon_list[[as.character(0.2)]], time_points=seq(0,3,0.003))
process_y_data_frame <- data.frame(y_for_epsilon_list[[as.character(0.2)]])
x_y_df <- data.frame(process_x_data_frame, process_y_data_frame)
x_y_df_2 <-melt(x_y_df,id.vars="time_points")
names(x_y_df_2)<-c("time_points","simulation_number","value")
x_y_df_2$process_group <- c(rep("X", 100100), rep("Y", 100100))

visualization_x_and_y <- ggplot(x_y_df_2,aes(x=time_points,y=value,colour=simulation_number)) + 
  geom_line() +
  theme(legend.position = "none") +
  labs(x ="time", y = "values of processes Xt and Yt") +
  theme(panel.background = element_rect(fill = "white", colour = "black",
                                        size = 0.25, linetype = "solid"))+
  theme(legend.background = element_rect(colour ="black")) 
